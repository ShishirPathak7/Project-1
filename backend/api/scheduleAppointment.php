...
if ($_SERVER['REQUEST_METHOD'] === 'GET') {
    try {
        ...
        if (!$patientId) {
            sendJSON(['error' => 'PatientID is required.'], 400);
        }

        // Fetch appointments for the specific patient
        $sql = "SELECT * FROM tblAppointments WHERE PatientID = ?";
    ...
    try {
        if ($action === 'add') {
            ...
            $sql = "INSERT INTO tblAppointments (PatientID, DoctorName, AppointmentDate, TimeSlot, Description, Status) VALUES (?, ?, ?, ?, ?, 'Scheduled')";
            $stmt = $conn->prepare($sql);
            $stmt->execute([
                $data['patientId'],
                $data['doctorName'],
            ...
            sendJSON(['message' => 'Appointment added successfully.']);
        } elseif ($action === 'edit') {
