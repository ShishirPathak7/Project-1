...
const PatientMyBlogs = () => {
  ...
  const fetchBlogs = async () => {
    try {
      ...
      const currentUserBlogs = data.filter(
        (blog) => blog.UserID === getCurrentUserId()
      );
      ...
      >
        <div className="blogs-page" style={{ padding: '20px' }}>
          ...
          <h1
            variant="h4"
            gutterBottom
            style={{ color: '#ffff', marginBottom: '20px', fontWeight: 'bold' }}
          >
            📖 My Blogs
          </h1>
          <Link
            to="/HealthcareProviderAddBlog"
            className="create-blog-btn"
            style={{
              textDecoration: 'none',
              color: '#fff',
          ...
          >
            {blogs.map((blog, index) => (
              ...
              >
                <Link
                  to={`/HealthcareProviderSingleBlogPage/${blog.BlogID}`}
                  style={{
                    textDecoration: 'none',
                    color: '#333',
                    width: '100%',
                ...
                >
                  ...
                  >
                    ...
                    >
                      Creator UserID: {blog.UserID}
                    </p>
                    <p
                      className="creation-date"
                      style={{ color: '#666', marginBottom: '10px' }}
                    >
                      Creation Date: {blog.CreationDate}
                    </p>
                    <p
